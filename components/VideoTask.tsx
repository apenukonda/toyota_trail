...
import { ChevronLeftIcon, PlayIcon, CheckIcon, CheckCircleIcon, XCircleIcon } from './icons';

type View = 'instructions' | 'modules' | 'videos' | 'player' | 'quiz' | 'completed';
...
const VideoTask: React.FC = () => {
  const { currentUser, setCurrentPage, updateTaskCompletion, getVideoProgress, updateVideoProgress, addScore, t, language } = useContext(AppContext);
                ...
                'onStateChange': (event: any) => {
                    if (event.data === (window as any).YT.PlayerState.ENDED) {
                        ...
                        setVideoProgress(prev => ({
                            ...prev,
                            [videoId]: { ...prev[videoId], watchedSeconds: duration, isComplete: true }
                        }));
                    }
                }
            }
  ...
  useEffect(() => {
    ...
    const progressInterval = setInterval(() => {
        if (playerRef.current && typeof playerRef.current.getCurrentTime === 'function' && playerRef.current.getPlayerState() === 1) { // 1 = playing
            ...
            setVideoProgress(prev => {
                const currentProgress = prev[videoId] || { watchedSeconds: 0, isComplete: false };
                if (currentTime > currentProgress.watchedSeconds) {
                    updateVideoProgress(videoId, currentTime, currentProgress.isComplete);
                    return { ...prev, [videoId]: { ...currentProgress, watchedSeconds: currentTime } };
                }
                return prev;
            });
        }
  ...
  const handleQuizComplete = useCallback(async (score: number) => {
    ...
    await updateVideoProgress(videoId, videoProgress[videoId]?.watchedSeconds || videoDuration, true);
    const newProgress = { ...videoProgress, [videoId]: { watchedSeconds: videoProgress[videoId]?.watchedSeconds || videoDuration, isComplete: true } };
    setVideoProgress(newProgress);

    setQuizScores(prev => {
      const updated = { ...prev, [videoId]: score };
      // Check if current module is fully completed
      const moduleVideoIds = currentModule.videos.map(v => v.id);
    ...
    const completedCount = Object.values(newProgress).filter(p => (p as { isComplete: boolean }).isComplete).length;
  ...
  }, [currentModuleIndex, currentVideoIndex, videoDuration, updateTaskCompletion, updateVideoProgress, videoProgress, currentQuizQuestions]);
  ...
  const confirmBack = () => {
    ...
    const currentVideo = currentModule.videos[currentVideoIndex];
    const videoId = currentVideo.id;
    const maxQuizScore = currentQuizQuestions.length > 0 ? currentQuizQuestions.length : 7;
    ...
    setQuizScores(prev => {
      ...
      ADVANCED_MODULES.forEach(module => {
        ...
        const moduleCompleted = moduleVideoIds.every(id => videoProgress[id]?.isComplete);
        if (moduleCompleted) {
  ...
  const isVideoWatched = currentVideoId && (videoProgress[currentVideoId]?.isComplete || ((videoProgress[currentVideoId]?.watchedSeconds || 0) >= videoDuration * 0.95 && videoDuration > 0));
  ...
  return (
    ...
    <div className={`w-full max-w-2xl text-center ${view === 'instructions' ? '' : 'hidden'}`}>
  <h1 className="text-4xl font-bold mb-4">{t('Advanced Quality Principles')}</h1>
  ...
  {t('Start Learning')}
      ...
      <div className={`w-full max-w-2xl text-center ${view === 'modules' ? '' : 'hidden'}`}>
  <h1 className="text-3xl font-bold mb-6">{t('Advanced Quality Principles')}</h1>
        <div className="space-y-4">
          {ADVANCED_MODULES.map((module, index) => {
            const moduleVideos = module.videos;
            const completedVideos = moduleVideos.filter(video => videoProgress[video.id]?.isComplete).length;
            const isModuleComplete = completedVideos === moduleVideos.length;
            return (
              <button key={module.id} onClick={() => selectModule(index)} disabled={isModuleComplete} className="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-md flex justify-between items-center text-left disabled:opacity-50 disabled:cursor-not-allowed hover:bg-red-100/50 transition-colors">
                 <div className="flex items-center">
                   <span className={`mr-4 font-bold ${isModuleComplete ? 'text-green-500' : 'text-red-500'}`}>
                     {isModuleComplete ? <CheckCircleIcon className="w-6 h-6"/> : module.id}
                   </span>
                   <span>{language === 'kn' ? (module.titleKn || module.title) : module.title}</span>
                 </div>
                {module.id === 'M3' ? <span className="font-bold text-blue-500">{t('Coming Soon')}</span> : isModuleComplete ? <span className="font-bold text-green-500">{t('done')}</span> : <span className="text-sm text-gray-500">{completedVideos}/{moduleVideos.length} {t('videos')}</span>}
              </button>
            )
          })}
        </div>
      ...
      <div className={`w-full max-w-2xl text-center ${view === 'videos' ? '' : 'hidden'}`}>
  <h1 className="text-3xl font-bold mb-6">{language === 'kn' ? (currentModule?.titleKn || currentModule?.title) : currentModule?.title}</h1>
        <div className="space-y-4">
          {currentModule?.videos.map((video, index) => {
            const progress = videoProgress[video.id];
            const isComplete = progress?.isComplete;
            return (
              <button key={video.id} onClick={() => selectVideo(index)} disabled={isComplete} className="w-full p-4 bg-white border border-gray-200 rounded-lg shadow-md flex justify-between items-center text-left disabled:opacity-50 disabled:cursor-not-allowed hover:bg-red-100/50 transition-colors">
                 <div className="flex items-center">
                   <span className={`mr-4 font-bold ${isComplete ? 'text-green-500' : 'text-red-500'}`}>
                     {isComplete ? <CheckCircleIcon className="w-6 h-6"/> : `V${index + 1}`}
                   </span>
                   <span>{language === 'kn' ? (video.titleKn || video.title) : video.title}</span>
                 </div>
                {isComplete ? <span className="font-bold text-green-500">{t('done')}</span> : <PlayIcon className="w-6 h-6" />}
              </button>
            )
          })}
        </div>
      ...
      <div className={`w-full max-w-4xl ${view === 'player' ? '' : 'hidden'}`}>
          <h2 className="text-2xl font-bold mb-4 text-center">
            {language === 'kn' ? (currentVideo?.titleKn || currentVideo?.title) : currentVideo?.title}
          </h2>
              ...
              {isVideoWatched ? (
                    <button onClick={() => {
                      if (currentQuizQuestions.length === 0) {
                        const quizQuestions = getRandomQuestions(currentModule?.quizzes[currentVideoIndex] || [], 7);
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400">{t('Watch to Unlock')}</p>
              )}
          </div>
      </div>

      {view === 'quiz' && (
        <Quiz questions={currentQuizQuestions} onComplete={handleQuizComplete} onScoreUpdate={setCurrentQuizScore} />
      )}
      ...
      {showBackWarning && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-fade-in">
          <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 text-center transform transition-all scale-95 opacity-0 animate-fade-in" style={{ animationDelay: '100ms', animationFillMode: 'forwards', animationName: 'zoomIn' }}>
            ...
            <h2 className="text-2xl font-extrabold text-black mb-4">{t('warning_title')}</h2>
